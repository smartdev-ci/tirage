<template>
  <div class="card">
    ...
    <div v-if="result" class="result" :class="alreadyDrawn ? 'warning' : 'success'">
      ...
      <button @click="downloadPDF" class="pdf-btn">📄 Télécharger en PDF</button>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    downloadPDF() {
      const doc = new jsPDF();
      doc.setFontSize(20);
      doc.text("🎯 Résultat du tirage", 20, 20);
      doc.setFontSize(16);
      doc.text(`Nom tiré par ${this.selectedUser} :`, 20, 40);
      doc.setFontSize(22);
      doc.setTextColor(0, 102, 204);
      doc.text(this.result, 20, 60);
      doc.save(`tirage_${this.selectedUser}.pdf`);
    }
  }
};
</script>
